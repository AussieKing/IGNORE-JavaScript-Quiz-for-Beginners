// ==================================================================================================
// overall stuff
var overallTimerLeft; // the countdown timer value in seconds
var overallTimer; // the timer object
var overallQuizRunning = false; // true if the quiz is running
function overallCountDownStart() {
  overallTimerLeft = 75;
  overallQuizRunning=true;
  overallTimer = setInterval(overallCountDown, 1000);
}

// called by timer every second
function overallCountDown() {
  if (overallTimerLeft <= 0) {
    document.getElementById("overall-timer").innerHTML = '0';
    overallStopCountDown();
    overallQuizRunning = false;
     // alert("Time's up!");
    // end quiz here
    showResult();
  } else {
    document.getElementById("overall-timer").innerHTML = overallTimerLeft;
    overallTimerLeft--;
  }
}

function overallPenalty() {
  overallTimerLeft -= 10; // subtract 10 seconds from the timer
  if (overallTimerLeft < 0) {
...
function overallStopCountDown() {
  clearInterval(overallTimer);
}

function overallGetScore() {
    // could be negative due to penalties, so return 0 if it is
    if (overallTimerLeft <= 0) {
        return 0;
    }
    return overallTimerLeft;
}
...
next_btn.onclick = ()=>{
  if (que_count < questions.length - 1 && overallQuizRunning ) { // if question count is less than total question length
...
function showResult() {
  ...
  document.getElementById('overall-score').textContent = 'Your Timer Score is: ' + overallGetScore();
}
